...
cat > bin/setenv.sh <<-END
	...
	JAVA_OPTS="-Xms2G -Xmx4G -Djgroups.bind_addr=127.0.0.1 \$JAVA_OPTS $JDK8_OPTS"
	export JAVA_OPTS
END
cd ..
...
wget --no-check-certificate https://repo1.maven.org/maven2/org/postgresql/postgresql/42.2.1/postgresql-42.2.1.jar
cd ../..
